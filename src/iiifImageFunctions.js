...
export function requestExceedsMaximum(imageInfo, width, height) {
  ...
  switch (version) {
    case 2: {
      const profile = getArrayFromInfoResponse(imageInfo, 'profile');
      return (profile && profile.length > 1 && profile[1].maxWidth
        && profile[1].maxWidth < width)
        || (profile && profile.length > 1 && profile[1].maxHeight
        && profile[1].maxHeight < height)
        || (profile && profile.length > 1 && profile[1].maxArea
        && profile[1].maxArea < width * height);
    }
    case 3: {
